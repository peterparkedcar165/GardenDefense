...
public class PoisonShroom : Shooter
{
    ...
    public override string GetPath2Description()
    {
        ...
        float durpl = PSData?.path2PoisonDurationPerLevel   ?? 1f;
        return $"Passive:\n\n" +
               $"Attacks apply a <color=purple>Poison</color> effect on hit for <color=green><b>{poisonDuration}</b></color> seconds, dealing <color=green><b>{PoisonBaseDPS + dpspl * effectivePath2Level:F0}</b></color> [<color=#FFB6C1><b>+{skillDamageMultiplier * magicPower:F0}</b></color>] <color=purple>Poison</color> damage per second.\n\n" +
