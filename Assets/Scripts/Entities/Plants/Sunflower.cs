...
public class Sunflower : Shooter
{
    ...
    public override void OnPath2Upgrade(int level)
    {
        ...
        sunProcChance = (SFData?.sunProcChance ?? 0.35f) + level * (SFData?.path2ProcChancePerLevel ?? 0.03f);
    }
    ...
    public override string GetPassiveDescription() =>
        $"Passively generates <color=green><b>{sunGenerated}</b></color> <color=yellow>Sun</color> for the garden every <color=green><b>{SunGenInterval:F1}</b></color> seconds. Attacks have a <color=green><b>{sunProcChance * 100f:F0}%</b></color> chance to reduce the cooldown by <color=green><b>1</b></color> second.";
    ...
    {
        int sunpl = SFData?.path2SunPerLevel ?? 2;
        float chancepl = SFData?.path2ProcChancePerLevel ?? 0.03f;
        ...
        string desc = details
            ? $"Passively generates <color=green><b>[({SFData?.baseSunGenerated ?? 6}) + ({sunpl}/Lvl.)]</b></color> <color=yellow>Sun</color> for the garden every <color=green><b>[({data.basePassiveCooldown:F0}) - (1/Lvl.)]</b></color> seconds. Attacks have a <color=green><b>[({(SFData?.sunProcChance ?? 0.35f) * 100f:F0}%) + ({chancepl * 100f:F0}%/Lvl.)]</b></color> chance to reduce the cooldown by <color=green><b>1</b></color> second."
        ...
        return $"Passive:\n\n{desc}\n\n" +
               ...
               $"Increase <color=green><b>Cooldown Reduction Chance</b></color> by <color=green><b>{chancepl * 100f:F0}%</b></color> per level. [<color=green><b>+{chancepl * effectivePath2Level * 100f:F0}%</b></color>]\n\n" +
