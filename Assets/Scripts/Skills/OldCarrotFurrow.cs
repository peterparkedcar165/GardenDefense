...
// oldCarrot ultimate: a square of churned earth plows along a line from the plant towards
// the target, damaging each insect exactly once as it passes over them. as the square
// enters each segment of the line it sprouts a oldCarrot visual there
public class OldCarrotFurrow : MonoBehaviour
{
    ...
    // stronger down the line along with each oldCarrot's own damage and size
    ...
    // the rendered world size of the oldCarrot visual at its authored prefab scale.
    // spacing derives from this so the oldCarrots always tile perfectly edge to edge
    ...
    public void Initialize(Vector3 origin, Vector2 direction, int segmentCount, float startOffset,
                           ...
                           OldCarrot source, GameObject squareVisualPrefab)
    {
        this.origin             = origin;
        ...
        // spacing follows the visual size of the oldCarrot (scaled by width level), so they still
    ...
    // path3 max: each oldCarrot further down the line hits harder and covers more ground than the
    // one before it, compounding off the PREVIOUS oldCarrot (1st = base, 2nd = +7%, 3rd = +7% on
    ...
    private void Update()
    {
        ...
        foreach (Insect insect in _scratch)
        {
            ...
            // pushed sideways, away from the line of oldCarrots (whichever side of it the insect is
            // already on), a distance equal to this oldCarrot's own width, rather than further
    ...
    private void SpawnSquareVisual(Vector3 position, float segmentScale)
    {
        ...
        // this segment's own growth) and upright, oldCarrots always render right side up. a small
