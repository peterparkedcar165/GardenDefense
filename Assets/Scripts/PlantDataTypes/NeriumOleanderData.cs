...
public class NeriumOleanderData : PlantData
{
    ...
    public override string GetAttackDescription() =>
        $"Fires a toxic petal at the target dealing <color=green><b>{baseAttackDamage:F0}</b></color> <color=purple>Poison</color> <color=#FFB6C1>Magic</color> damage. The petal bounces to <color=green><b>{baseBounceCount}</b></color> additional target(s).";
    ...
    public override string GetSkillDescription() =>
        $"Target an area. Instantly deals <color=green><b>{baseSkillFlatDamage:F0}</b></color> <color=purple>Poison</color> <color=#FFB6C1>Magic</color> damage to all insects within <color=green><b>{baseSkillRadius}</b></color> radius, rooting them and applying <color=#9B59B6>Oleandic Toxin</color>.";
}
