...
public class FrostbiteEffect : DoTEffect
{
    ...
    public override string GetDescription() => $"Deal (<color=green>{healthPerSecond * 100}%</color> Max Health) + (<color=green>{adPerSecond * 100}%</color> Attack Damage) <color=#00BFFF>Ice</color> Physical damage per second. Reduces Tenacity by <color=green>{tenacityReduction * 100}%</color>.";

    public override void OnApply()
    {
        base.OnApply();
        damagePerSecond = (healthPerSecond * target.maxHealth) + (adPerSecond * source.attackDamage) + 2f;
